      ...
      <div className="flex-1 p-4 overflow-y-auto flex flex-col gap-2">
        {messages.map((m, i) => (
          <div key={i} className={m.from === "bot" ? "text-left" : "text-right"}>
            <div className={`inline-block px-3 py-2 rounded-lg ${m.from === "bot" ? "bg-gray-100" : "bg-blue-600 text-white"}`}>
              {m.text}
            </div>
          </div>
        ))}
      ...
      <div className="p-2 border-t flex flex-col gap-2">
        {predefinedQA.map((qa, i) => (
          <button
            ...
            className="text-left px-3 py-2 rounded-lg bg-gray-50 hover:bg-gray-100 text-sm"
          >
            {qa.question}
          </button>
        ))}
